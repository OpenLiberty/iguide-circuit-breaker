...
state the output immediately displays a slightly different response of
...
checkBalance service so this request will be successful and shows   
...
failures. The state changes are the same as described above except that we now
have a fallback method assigned so the output will immediately display the 
output of the fallback whenever a request fails or if the circuit is in an open
state. The output from the fallback shows
	  "The last known balance of your account is $10,293"

To restart the application in order to simulate the 2 failing requests again
you can stop and restart the circuitBreakerSampleServer as indicated above.
...
Otherwise, stop the circuitBreakerSampleServer as indicated above, run the Maven 
